...
try {
  ...
  switch ($_GET['view']) {
    case '':
      // get promoted offers
      $promoted_offers = [];
      $get_promoted = $db->query("SELECT posts.post_id FROM posts INNER JOIN posts_offers ON posts.post_id = posts_offers.post_id WHERE posts.in_group = '0' AND posts.in_event = '0' AND posts.post_type = 'offer' AND posts.boosted = '1' AND (posts.pre_approved = '1' OR posts.has_approved = '1') ORDER BY RAND() LIMIT 3") or _error('SQL_ERROR');
  ...
  $total = $db->query("SELECT COUNT(*) as count " . $distance_max_clause . " FROM posts INNER JOIN posts_offers ON posts.post_id = posts_offers.post_id " . $distance_join . " WHERE posts.in_group = '0' AND posts.in_event = '0' AND (posts.pre_approved = '1' OR posts.has_approved = '1')" . $where_query . $distance_query . $order_query) or _error('SQL_ERROR');
  ...
  $get_rows = $db->query("SELECT posts.post_id " . $distance_clause . " FROM posts INNER JOIN posts_offers ON posts.post_id = posts_offers.post_id " . $distance_join . " WHERE posts.in_group = '0' AND posts.in_event = '0' AND (posts.pre_approved = '1' OR posts.has_approved = '1')" . $where_query . $distance_query . $order_query . $limit_query) or _error('SQL_ERROR');
