...
try {
  ...
  switch ($_POST['type']) {
    ...
    case 'market':
      /* get the request */
      $get_withdrawal_request = $db->query(sprintf("SELECT * FROM market_payments WHERE payment_id = %s", secure($_POST['id'], 'int'))) or _error('SQL_ERROR_THROWEN');
      if ($get_withdrawal_request->num_rows == 0) {
        _error(400);
      }
      $withdrawal_request = $get_withdrawal_request->fetch_assoc();
      ...
      switch ($_POST['handle']) {
        case 'approve':
          ...
          process_automatic_withdrawal($withdrawal_request['method'], $withdrawal_request['amount'], $withdrawal_request['method_value']);
          /* approve request */
          $db->query(sprintf("UPDATE market_payments SET status = '1' WHERE payment_id = %s", secure($_POST['id'], 'int'))) or _error('SQL_ERROR_THROWEN');
          /* send notification to request author */
        ...
        case 'decline':
          ...
          $db->query(sprintf("UPDATE market_payments SET status = '-1' WHERE payment_id = %s", secure($_POST['id'], 'int'))) or _error('SQL_ERROR_THROWEN');
          /* update user balance */
          $db->query(sprintf("UPDATE users SET user_market_balance = user_market_balance + %s WHERE user_id = %s", secure($withdrawal_request['amount']), secure($withdrawal_request['user_id'], 'int'))) or _error('SQL_ERROR_THROWEN');
          /* send notification to request author */
