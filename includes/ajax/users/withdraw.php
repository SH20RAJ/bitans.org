...
try {
  ...
  switch ($_GET['type']) {
    ...
    case 'market':
      ...
      if (!$user->_data['can_sell_products'] || !$system['market_money_withdraw_enabled']) {
      ...
      // process
      /* insert payment */
      $db->query(sprintf("INSERT INTO market_payments (user_id, amount, method, method_value, time, status) VALUES (%s, %s, %s, %s, %s, '0')", secure($user->_data['user_id'], 'int'), secure($_POST['amount']), secure($_POST['method']), secure($_POST['method_value']), secure($date))) or _error('SQL_ERROR_THROWEN');
      /* update user balance */
      $db->query(sprintf("UPDATE users SET user_market_balance = user_market_balance - %s WHERE user_id = %s", secure($_POST['amount']), secure($user->_data['user_id'], 'int'))) or _error('SQL_ERROR_THROWEN');
