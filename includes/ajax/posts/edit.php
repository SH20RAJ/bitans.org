  ...
  switch ($_POST['handle']) {
    ...
    case 'product':
      ...
      if ($_POST['is_digital'] && is_empty($_POST['product_url']) && is_empty($_POST['product_file'])) {
        return_json(array('error' => true, 'message' => __("Please add your product file or download URL")));
      }
      /* check product download URL */
      if (!is_empty($_POST['product_url']) && !valid_url($_POST['product_url'])) {
        return_json(array('error' => true, 'message' => __("Please add valid product download URL")));
      }
      /* check product name */
    ...
    case 'offer':
      ...
      if (!is_empty($_POST['end_date']) && strtotime(set_datetime($_POST['end_date'])) <= strtotime($date)) {
        return_json(array('error' => true, 'message' => __("End date must be after today datetime")));
      }
      ...
      if (!is_empty($_POST['price']) && (!is_numeric($_POST['price']) || $_POST['price'] <= 0)) {
        return_json(array('error' => true, 'message' => __("Enter valid price for your offer")));
      }
