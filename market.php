...
try {
  ...
  switch ($_GET['view']) {
    case '':
      ...
      $promoted_products = [];
      $get_promoted = $db->query("SELECT posts.post_id FROM posts INNER JOIN posts_products ON posts.post_id = posts_products.post_id WHERE posts.post_type = 'product' AND posts_products.available = '1' AND posts.boosted = '1' AND (posts.pre_approved = '1' OR posts.has_approved = '1') ORDER BY RAND() LIMIT 3") or _error('SQL_ERROR');
  ...
  if (in_array($view, ['', 'search', 'category'])) {
    ...
    $get_rows = $db->query("SELECT posts.post_id " . $distance_clause . " FROM posts INNER JOIN posts_products ON posts.post_id = posts_products.post_id INNER JOIN users ON posts.user_id = users.user_id WHERE posts.post_type = 'product' AND posts_products.available = '1' AND (posts.pre_approved = '1' OR posts.has_approved = '1')" . $where_query . $distance_query . $order_query . $limit_query) or _error('SQL_ERROR');
