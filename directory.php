...
try {
  ...
  switch ($_GET['view']) {
    ...
    case 'users':
      ...
      $total = $db->query("SELECT COUNT(*) as count FROM users WHERE users.user_banned = '0'") or _error('SQL_ERROR');
      ...
      // get users
      $rows = [];
      $get_rows = $db->query("SELECT * FROM users WHERE users.user_banned = '0' " . $limit_query) or _error('SQL_ERROR');
    ...
    case 'posts':
      ...
      // get posts
      $rows = [];
      $get_rows = $db->query("SELECT posts.post_id FROM posts LEFT JOIN users ON posts.user_id = users.user_id AND posts.user_type = 'user' LEFT JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = 'page' LEFT JOIN `groups` ON posts.in_group = '1' AND posts.group_id = `groups`.group_id LEFT JOIN `events` ON posts.in_event = '1' AND posts.event_id = `events`.event_id WHERE posts.privacy = 'public' AND (posts.pre_approved = '1' OR posts.has_approved = '1') AND NOT (users.user_name <=> NULL AND pages.page_name <=> NULL) " . $limit_query) or _error('SQL_ERROR');
      while ($row = $get_rows->fetch_assoc()) {
